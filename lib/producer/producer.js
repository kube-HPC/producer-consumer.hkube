...
class Producer extends EventEmitter {
    ...
    _checkStalledJobs() {
        if (this._setting.enableCheckStalledJobs) {
            this._checkStalledJobsInterval = this._checkStalledJobsInterval.bind(this);
            this._checkStalledJobsInterval();
        }
    }

    async _checkStalledJobsInterval() {
        try {
            ...
            setTimeout(this._checkStalledJobsInterval, this._setting.checkStalledJobsInterval);
        }
    }
    ...
    createJob(options) {
        return new Promise((resolve, reject) => {
            ...
            const queue = this._createQueue(options.job.type, options.job.prefix);
            let job;
            let timer;
    ...
    _createQueue(jobType, jobPrefix) {
