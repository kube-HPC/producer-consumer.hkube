...
const validator = new Validator({ useDefaults: true, coerceTypes: true, nullable: true });
...
class ProducerClass extends EventEmitter {
    ...
    createJob(jobOptions) {
        ...
        const { id, type, prefix, data, options } = jobOptions;
        ...
        if (this._tracer) {
            ...
            jobOptions.data.spanId = span.context();
        }
        ...
        if (options.waitingTimeout > 9999999) {
            timer = setTimeout(() => {
                if (job) {
                    job.discard();
                    job.remove();
        ...
        return this._producer.createJob({ name, jobId, type, prefix, data, options });
    }
