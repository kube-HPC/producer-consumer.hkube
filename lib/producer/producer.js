...
class Producer extends EventEmitter {
    ...
    _checkJobs() {
        this._checkJobsInterval(this._setting.checkCrashedJobsInterval, this._checkCrashedJobs.bind(this));
    ...
    async _checkCrashedJobs(queue) {
        const failed = await queue.getFailed(0, 1000000);
        failed.forEach((j) => {
            const { jobId } = j.data;
            this.stopJob({ type: j.name, jobId: j.id });
    ...
    async _checkStalledJobs(queue) {
        ...
        const waiting = await queue.getWaiting(0, 1000000);
        waiting.filter(w => w.processedOn).forEach((j) => {
            const { jobId } = j.data;
            const jobData = this._createJobData(jobId, j.data, 'StuckJob');
